...
`slug` con `courseSlug` (carpeta contenedora + basename del `.docx`, `SPEC 09`)
y el hash sha256 del `.docx` (`hashFile`), y busca el slug en
`<workspaceRoot>/course-registry.json` (`SPEC 07`):
...
- **Slug nuevo**: requiere `--area` y `--plataforma` (error de uso claro
  ...
  palabra capitalizada — salvo que se pase `--titulo`. Excepción: si el
  basename del `.docx` (sin extensión, sin acentos, case-insensitive) es
  exactamente `"clase"` o `"clases"` — nombre genérico que no aporta
  información — el título default se genera desde `courseFolderName`
  (la carpeta contenedora "prettified") en vez del basename del archivo
  (`SPEC 09`); `--estado` por
...
| `modules.ts` | Divide las unidades en módulos por cada H1 (`splitModules`) y detecta/descarta el módulo de índice o tabla de contenido (`isIndexModule`, `removeIndexModule`), además de `slugify` (slug de módulo). `courseFolderName` (basename de la carpeta contenedora del `.docx`) y `courseSlug` (`slugify` de carpeta+basename del `.docx`) calculan el slug de **curso**: incorporan la carpeta contenedora para que dos cursos distintos cuyo `.docx` comparte nombre de archivo en carpetas distintas no colisionen bajo el mismo slug (`SPEC 09`). |
...
| `sync-course.ts` | Entrypoint de `npm run sync-course -- <archivo.docx> [MODULO_N] --area <area> --plataforma <plataforma> [--estado <estado>] [--titulo <titulo>] [--dry-run] [--force]` (`SPEC 08`). `parseArgv`, `prettifyTitle` (basename del `.docx` a título "prettified", sin cambios) y `runSyncCourse` (resuelve el `parent` automáticamente vía `course-registry.json`, creando la fila en `Cursos` con `createCourse` si el slug es nuevo, y delega siempre en `runSync` de `sync.ts` — sin modificarlo — para subir los módulos) separados de `main()`. No acepta `PARENT_PAGE_ID` como argumento; ver el detalle de la orquestación en "Instalar y correr". El título default de un curso nuevo aplica `prettifyTitle` sobre el basename del `.docx`, **salvo** que ese basename (normalizado) sea `"clase"` o `"clases"` — un nombre genérico de archivo (`GENERIC_BASENAMES`, interno) — en cuyo caso aplica `prettifyTitle` sobre `courseFolderName(docxPath)` (la carpeta contenedora) en su lugar; `--titulo` explícito sigue pisando el default en ambos casos (`SPEC 09`). |
